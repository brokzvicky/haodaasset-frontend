...
// ─────────────────────────────────────────────────────────────────
// My Attendance Page (Attendance Management module — self-service)
// ─────────────────────────────────────────────────────────────────
export function EmployeeAttendance() {
  const { data: records, loading, error } = useGet("/attendance");

  const list = records || [];
  const todayStr = new Date().toDateString();
  const todaysPunches = list.filter((r) => new Date(r.punchTime).toDateString() === todayStr);
  const lastIn = [...todaysPunches].reverse().find((r) => (r.punchType || "").toUpperCase() === "IN");
  const lastOut = todaysPunches.find((r) => (r.punchType || "").toUpperCase() === "OUT");
  ...
  return (
    <Layout title="My Attendance" subtitle="Your biometric punch history from the eSSL attendance devices">
      <ErrorBanner message={error} />

      {!loading && (
        <div style={{ display: "flex", gap: 16, marginBottom: 20, flexWrap: "wrap" }}>
          {[
            { label: "Punches Today", value: todaysPunches.length, color: "#1a56db" },
            { label: "First In Today", value: lastIn ? formatTime(lastIn.punchTime).split(",").pop().trim() : "—", color: "#059669" },
            { label: "Last Out Today", value: lastOut ? formatTime(lastOut.punchTime).split(",").pop().trim() : "—", color: "#b45309" },
          ].map((s) => (
            <div key={s.label} className="card" style={{
      ...
      <div className="card">
        <div className="card-header">
          <div>
            <div className="card-title">Punch History</div>
            <div className="card-subtitle">{loading ? "Loading…" : `${list.length} record(s)`}</div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-state" style={{ padding: "56px 20px" }}>
            ...
            <div className="empty-sub">
              Once your device PIN is mapped to your employee profile by IT/HR,
              <br />your fingerprint punches will appear here automatically.
            </div>
          </div>
        ) : (
          <div className="table-wrap">
            <table className="data-table">
              <thead>
                <tr><th>Punch Time</th><th>Type</th><th>Verify Mode</th><th>Device</th></tr>
              </thead>
              <tbody>
                {list.map((r) => (
                  <tr key={r.id}>
                    <td>{formatTime(r.punchTime)}</td>
                    <td>
                      <span style={{
                        fontSize: 11.5, fontWeight: 700, padding: "3px 10px", borderRadius: 999,
                        background: (r.punchType || "").toUpperCase() === "IN" ? "#dcfce7" : "#fee2e2",
                        color: (r.punchType || "").toUpperCase() === "IN" ? "#166534" : "#991b1b",
                      }}>
                        {(r.punchType || "UNKNOWN").toUpperCase()}
                      </span>
                    </td>
                    <td>{r.verifyMode || "—"}</td>
                    <td>{r.deviceName || "—"}</td>
